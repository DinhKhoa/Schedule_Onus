...
exports.login = async (req, res, next) => {
  try {
    ...
    // Check admin first
    const admin = await QuanTriVien.findOne({ taiKhoan: soDienThoai });
    if (admin) {
      ...
      return res.json({ token, user: { id: admin._id, hoTen: admin.hoTen, vaiTro: 'ADMIN' } });
    }

    // Check user (HOIVIEN / PT)
    const user = await User.findOne({ soDienThoai });
    if (!user) return res.status(401).json({ error: 'Tài khoản không tồn tại' });
    if (user.trangThai !== 'HoatDong') return res.status(403).json({ error: 'Tài khoản đã bị khóa' });
    ...
    res.json({ token, user: { id: user._id, hoTen: user.hoTen, vaiTro: user.vaiTro } });
  } catch (error) {
    next(error);
  }
};
